...
**⚠️ UNRESOLVED as of end of session (2026-07-21): Day view (`TimelineCalendar.tsx`) still renders completely blank on a real device** — no hour gutter, no gridlines, no appointment blocks, nothing below Calendar's header/date-row/banners, even after three separate fix attempts, each confirmed via `tsc --noEmit` but none confirmed working live:
1. Added `style={{flex:1}}` to the outer `ScrollView` (the same fix that *did* work for `MultiDayView`/`AgendaView`/the 6 other pull-to-refresh screens) — didn't fix Day view.
2. Suspected the `GestureDetector` (pinch-zoom) wrapper was interfering with flex-layout propagation to its `ScrollView` child; restructured so `TimelineCalendar` returns to its original pre-redesign root shape (`GestureDetector` → `ScrollView` directly, no extra wrapping `View`), moving the refresh-heart overlay up into `calendar.tsx` instead — didn't fix it either.
3. Found the inner horizontal `ScrollView` (holding the actual grid columns) had no `width`/`flex` at all, sitting as the second item in a `flexDirection: 'row'` content container — gave it `style={{flex:1}}` — still blank after reload.

**Root cause not actually found.** Week/Agenda/Month views all work correctly with the same underlying data (confirmed via the "10 open slots"/"Quiana Jones" appointment showing correctly in Week view for the same date that Day view shows nothing for) — so this is isolated to `TimelineCalendar.tsx` specifically, most likely something about the `GestureDetector`(pinch) + nested-`ScrollView` + Android combination that wasn't correctly diagnosed. **User has decided to stop here and address this in its own dedicated future session** rather than continue guessing, since the Calendar screen is expected to be redone again anyway. Do not assume any of the 3 attempted fixes actually resolved anything — start the next Calendar session by reproducing this blank-Day-view bug fresh and investigating with real device tools (React DevTools element inspector, or temporarily stripping the `GestureDetector`/pinch gesture entirely to isolate whether that's really the cause) rather than further blind layout edits.

`tsc --noEmit` clean. **Not yet visually verified on a real device/emulator.**
