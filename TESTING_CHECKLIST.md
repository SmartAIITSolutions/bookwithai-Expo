...
- 🔧 **Google Sign-In still spun forever after real Google consent completed** (confirmed live on production build -- got the "you shared account data" email from Google, app never picked it up). Root cause: `WebBrowser.openAuthSessionAsync()`'s resolved result was never firing on Android, because the app's own deep-link handling (the same system that used to show "Unmatched Route" for this exact URL, then landed on the `auth/callback.tsx` spinner after that fix) was claiming the redirect intent first, leaving `openAuthSessionAsync()`'s promise permanently unresolved. Fixed by moving the actual code exchange out of `handleGoogleSignIn()` (`auth/index.tsx`) and into `_layout.tsx`'s central `handleDeepLink()`, the same proven-reliable path already used for staff invites and password resets -- `handleGoogleSignIn()` now just opens the browser and doesn't depend on its return value. **Not yet retested** -- session ended before confirming on emulator.
- 🐛 **Magic Link reported with the same "sent, but can't get back into the app" symptom** -- uses the identical `auth/callback` redirect target, so the Google Sign-In fix above may already cover it, but this was NOT confirmed before the session ended. Also genuinely possible this is actually the same *separate*, already-known issue as the Forgot Password link (Gmail wraps links in its own `google.com/url?q=...` redirector, which can mangle the deep link's parameters before the app ever sees them) -- needs a clean retest specifically checking whether the link was opened directly in the Gmail app (more likely to hit the wrapping issue) vs. another route, before concluding which bug this is.
